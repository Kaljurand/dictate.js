(function(window){
	...
	// Defaults
	//var SERVER = "ws://bark.phon.ioc.ee:82/dev/ws-speech/speech";
	...
	var Dictate = function(cfg) {
			...
			if (ws) {
				...
				if (state == 1) {
					...
					if (item instanceof Blob) {
						if (item.size > 0) {
							ws.send(item);
							config.onEvent(MSG_SEND, 'Send: blob: ' + item.type + ', size: ' + item.size);
						} else {
							config.onEvent(MSG_SEND_EMPTY, 'Send: blob: ' + item.type + ' (EMPTY, NOT SENDING)');
						}
		...
		function createWebSocket() {
			...
			ws.onmessage = function(e) {
				...
				if (data instanceof Object && ! (data instanceof Blob)) {
					...
					// TODO: only if the string is trusted
					var res = eval("(" + data + ")");
					if (res.status == 0) {
						if (res.result.final) {
		...
		function monitorServerStatus() {
			var ws = new WebSocket(SERVER_STATUS);
			ws.onmessage = function(evt) {
				var json = JSON.parse(evt.data);
				config.onServerStatus(json);
			};
		}
